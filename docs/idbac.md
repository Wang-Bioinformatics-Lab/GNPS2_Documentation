1) Convert raw autoExecute data (Bruker FID) to mzML via MSConvert. 
...
2) Upload your converted mzML file along with a MALDI plate map labeled with your isolate IDs (CSV or XLSX) to "File Browser". 
...
3) Split MALDI Workflow submission (https://gnps2.org/workflowinput?workflowname=split_maldi_workflow): Select your autoExecute mzML and plate map from the drop down menu. The output of this workflow is individual, labeled mzML files based on MALDI well-position. 
...
4) IDBac Deposition (https://gnps2.org/workflowinput?workflowname=idbacdeposition_workflow): Upload the mzML files from the previous step with the cooresdponding metadata sheet. Ensure "Dryrun mode" is set to "No". 
...
5) Analysis of strains against IDBac seed database (https://gnps2.org/workflowinput?workflowname=idbac_analysis_workflow): To compare isolates with database seeds (spectra of genetically verified strains), upload your mzML files with the corresponding metadatasheet. Choose your prefered parameters from the drop down menus. 

![IdBac analysis ](https://github.com/Wang-Bioinformatics-Lab/GNPS2_Documentation/assets/140128524/346b21c3-10d0-4c1b-9c8a-422f6649a5fe)
